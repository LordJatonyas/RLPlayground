# From Johnny Code's "Build a Real-World Reinforcement Learning Environment" tutorial series
# Original environment name: airplane_boarding
...
# Register this module as a gym env. Once registered, the id is usable in gym.make()
# When running this code, you can ignore this warning: "UserWarning: WARN: Overriding environment PlaneBoarding-v0 already in registry."
register(
    id = "PlaneBoarding-v0",
    entry_point = "Environments.PlaneBoarding.planeboarding:PlaneEnv" # module_name: class_name
)
...
class Passenger:
    def __init__(self, seat_num, row_num):
        ...
        self.status = PassengerStatus.MOVING # default status

    def __str__(self):
        return f"P{self.seat_num:02d}"

# Passengers wait at the lobby in rows
# We assume that the lobby is like a grid
class LobbyRow:
...
class Lobby:
    def __init__(self, nrows, seats_per_row):
        self.nrows = nrows
        self.seats_per_row = seats_per_row
        self.lobby_rows = [LobbyRow(row_num, self.seats_per_row) for row_num in range(self.nrows)]

    def rm_passenger(self, row_num):
        passenger = self.lobby_rows[row_num].passengers.pop()
        return passenger

    def count_passengers(self):
        count = 0
        for r in self.lobby_rows:
            count += len(r.passengers)
        return count

class BoardingLine:
    def __init__(self, nrows):
        self.nrows = nrows
        self.line = [None for i in range(nrows)]
    ...
    def num_passengers_stalled(self):
        count = 0
        for p in self.line:
            if p is not None and p.status == PassengerStatus.STALLED:
                count += 1
        return count
    ...
    def num_passengers_moving(self):
        count = 0
        for p in self.line:
            if p is not None and p.status == PassengerStatus.MOVING:
                count += 1
        return count
    ...
    def move_forward(self):
        for i, p in enumerate(self.line):
            ...
            if p is None or i == 0 or p.status == PassengerStatus.STOWING:
                continue

            # Move passenger forward if no one is blocking
            if (p.status == PassengerStatus.STALLED or p.status == PassengerStatus.MOVING) and self.line[i - 1] is None:
                p.status = PassengerStatus.MOVING
                self.line[i - 1] = p
                self.line[i] = None
            else:
                p.status = PassengerStatus.STALLED
        
        # Truncate the empty spots at the end of the line
        for i in range(len(self.line) - 1, self.nrows - 1, -1):
...
class PlaneEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, nrows=3, seats_per_row=5):
        self.seats_per_row = seats_per_row
        self.nrows = nrows
        self.nseats = nrows * seats_per_row

        self.render_mode = render_mode

        # Define the action space
        self.action_space = spaces.Discrete(self.nrows)

        # Define the observation space
        # The observation space is used to validate the observation returned by reset() and step()
        # e.g. [0, -1, 1, -1, 2, -1, ..., 6, 2, 7, 1, ...]
    ...
    def reset(self, seed=None, options=None):
        ...
        self.plane_rows = [PlaneRow(row_num, self.seats_per_row) for row_num in range(self.nrows)]
        self.lobby = Lobby(self.nrows, self.seats_per_row)
        self.boarding_line = BoardingLine(self.nrows)
    ...
    def _get_observation(self):
        ...
        for i in range(len(self.boarding_line.line), self.nseats):
            observation.append(-1)
            observation.append(-1)
        return np.array(observation, dtype=np.int32)

    def step(self, row_num):
        assert row_num >= 0 and row_num < self.nrows, f"Invalid row number {row_num}"
        
        reward = 0

        passenger = self.lobby.rm_passenger(row_num)
        ...
        if self.lobby.count_passengers() > 0:
            ...
            # No more passengers in the lobby, so no more actions to choose from, move line until all passengers are seated
        ...
        # Gym requires returning the observation, reward, terminated, truncated, and info dict
    ...
    def _move(self):
        for row_num, passenger in enumerate(self.boarding_line.line):
            ...
            # If outside of plane's aisle
            if row_num >= len(self.plane_rows):
                break
            
            # Try to sit passenger, if successful, remove from line
            if self.plane_rows[row_num].try_sit_passenger(passenger):
    ...
    def _render_terminal(self):
        ...
        for row in self.plane_rows:
            ...
            print()
        print("\nLine entering plane:")
        for i in range(self.nrows, len(self.boarding_line.line)):
...
def my_check_env():
    ...
    env = gym.make("PlaneBoarding-v0", render_mode=None)
    check_env(env.unwrapped)

if __name__ == "__main__":
    # my_check_env()
    env = gym.make("PlaneBoarding-v0", render_mode="terminal")
